...
BOOL CCodeTesterDlg::OnInitDialog()
{
	...
	*/
	...
	if(i == 1)
	{
		m_CmbInput.AddString(L"MsgSectionInit(\"192.168.0.27\",0x00010001, 0x04, 0x05)");
		m_CmbInput.AddString(L"CallStart(\"1234\",\"192.168.0.27\")");
	}
...
{
	/*
	if(g_CmdParser.m_CmdTableCount == 0)
	{
		MessageBox(L"CmdParser 没有初始化\n");
		return ;
	}
	...
	CmdParser_Reset(&g_CmdParser);
	CmdParser_Parse(&g_CmdParser, (uint8*)pChar, strA.GetLength());
	CmdParser_Excute(&g_CmdParser);
	*/
}
